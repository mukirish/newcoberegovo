...
public partial class bitrix_admin_IBlockImportXML : BXAdminPage, ICallbackEventHandler
{
    ...
    {
        return new ExecuteDelegate(this.DoImport).BeginInvoke(this.CallBackMethod, null);
    }
    ...
    public string GetCallbackResult()
    {
        ...
        {
            progressFormat = "{0}<br>"+
                            GetMessageRaw("Progress.Section") + ": {1}<br>" +
                            GetMessageRaw("Progress.Elements") + ": {2}<br>" +
                            GetMessageRaw("Progress.Total") + ": {3:p}";
        }
