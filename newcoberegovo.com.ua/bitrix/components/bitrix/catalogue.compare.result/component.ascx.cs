...
namespace Bitrix.IBlock.Components
{
	public partial class CatalogueCompareResultComponent : BXComponent
	{
		...
		protected override void OnLoad(EventArgs e)
		{
			...
			bool cached = IsCached(BXPrincipal.Current.GetAllRoles(true), ((BXIdentity)BXPrincipal.Current.Identity).Id, ((BXIdentity)BXPrincipal.Current.Identity).Id == 0 && Session != null ? Session.SessionID : null);
			...
			using (BXTagCachingScope cacheScope = BeginTagCaching())
			{
				...
				if(this.ShouldBeCached)
					Results[guidKey] = newGuid = Guid.NewGuid().ToString();
		...
		{
			if (this.Session != null && this.newGuid != null)
			{
				Session[guidKey] = newGuid;
			}
			base.Render(writer);
		}
