...
using BXUndoPageCreationOperation = Bitrix.Mvc.Undo.BXUndoPageCreationOperation;
using System.Web.Hosting;

public partial class bitrix_dialogs_NewPageWizardMvc : Bitrix.UI.BXDialogWizardPage
{	
	...
	private BXParamsBag<string> _keywordsTypes;
	private Dictionary<string, Dictionary<string, string>> _keywords;
	private Dictionary<string, string> _menuTypeDic;
		...
		try
		{
			...
			else
			{
				...
				else
				{
					...
					if (!IsPostBack)
					{
						if (!string.IsNullOrEmpty(fi.Extension) && string.Compare(".cshtml", fi.Extension, StringComparison.OrdinalIgnoreCase) == 0)
			...
			if (!IsPostBack)
			{
				...
				if (_clientFileName.Length < ".cshtml".Length)
					throw new InvalidOperationException(string.Format("Invalid file name has been detected: '{0}'!", _clientFileName));

				tbxPageName.Text = _clientFileName.Remove(_clientFileName.Length - ".cshtml".Length);
			...
			newPageWizardLong.EnableFormIdReplace = false;			
		}
	...
	{
		BXPublicMenuItemCollection menuItemCol = BXPublicMenu.Menu.GetMenuByUri(menuType, BXRoutesHelper.MakeVirtualUrlPathForPagePath(virtualPath));
	...
	protected override void OnLoad(EventArgs e)
	{
		...
		try
		{
			...
			DescriptionInfoParagraphs.Add(string.Format("<a href=\"{0}?path={1}&new=\">{2}</a>", VirtualPathUtility.ToAbsolute("~/bitrix/admin/FileManEditMvc.aspx"), HttpUtility.UrlEncode(_clientVirtualDirPath), GetMessage("CreatePageInControlPanel")));
	...
	void wizard_ButtonClick(object sender, BXPageAsDialogButtonClickEventArgs e)
	{
		try
		{
			...
			{
				_clientFileName = string.Format("{0}.cshtml", _clientFileName);
			}
			else if (string.Compare(fnExt, ".cshtml", StringComparison.Ordinal) != 0)
			{
				_clientFileName = string.Format("{0}.cshtml", _clientFileName);
			}

			//имя 
			string pageNameExtensionless = _clientFileName.Remove(_clientFileName.Length - ".cshtml".Length);
			...
			//string content = Bitrix.Mvc.IO.BXSecureIO.SaveCshtml(_clientVirtualPath, Encode(tbxTitle.Text), this.siteId);
			...
			BXSecureIO.FileWriteAllText(_clientVirtualPath, Bitrix.Mvc.UI.Admin.VirtualSite.Templates.Razor.BuildCshtml(tbxTitle.Text, null, tbxTitle.Text));
			
			var controllerPath = BXRoutesHelper.ReplaceExtension(this._clientVirtualPath, ".cshtml", ".bxc");

			var undoOperation = new BXUndoPageCreationOperation { FileVirtualPath = this._clientVirtualPath };



			if (!BXSecureIO.FileExists(controllerPath))
			{
				undoOperation.AdditionalVirtualPaths.Add(controllerPath);
				BXSecureIO.FileWriteAllText(controllerPath, Bitrix.Mvc.UI.Admin.VirtualSite.Templates.BXC.GetEmptyBXC(keywordBag));
			}
			else
			{
				try
				{
					var data = BXComponent2Loader.LoadComponentsData(HostingEnvironment.MapPath(controllerPath));
					data.Attributes = keywordBag.ToList((k, v) => new BXContainerAttributeXmlItem { Id = k, Value = v });
					BXSecureIO.FileWriteAllText(controllerPath, data.ToXml());
				}	
				catch
				{					
				}
			...
			if (chbxEditAfterSave.Checked)
			{
				...
				SwitchToDialog(
					string.Format(
						"{0}?path={1}&clientType=WindowManager&forcedRedirection=&noundo=", 
						VirtualPathUtility.ToAbsolute("~/bitrix/dialogs/VisualPageEditorMvc.aspx"), 
						HttpUtility.UrlEncode(_clientVirtualPath)
					), 
					dlgSetting, 
					GetMessage("NewPageIsSuccessfullyCreatedShowEditDialog"), 
					BXDialogGoodbyeWindow.LayoutType.Success, 
					2000
				);				
				return;
			}
			Redirect(BXRoutesHelper.MakeAbsoluteUrlPathForPagePath(_clientVirtualPath), string.Empty, BXDialogGoodbyeWindow.LayoutType.Success);			
		}
	...
	private void AddToMenu(string menuTypeId, MenuTypeInfo info, string pageNameExtensionless)
	{
		...
		var menuItem = new BXPublicMenuItem { Link = BXRoutesHelper.MakeVirtualUrlPathForPagePath(this._clientVirtualPath) };
