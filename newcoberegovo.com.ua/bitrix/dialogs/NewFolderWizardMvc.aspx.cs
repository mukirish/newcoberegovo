...
public partial class bitrix_dialogs_NewFolderWizardMvc : Bitrix.UI.BXDialogWizardPage
{
	const string _defaultPageName = "Default";

	private string siteId = string.Empty;
	private string _defaultPathSite = "~/Sites/{0}/";
	...
	protected override void OnInit(EventArgs e)
	{
		try
		{
			...
			else
			{
				...
				if (di.Parent == null)
				{
					this.Close(string.Format("{0}: '{1}'", this.GetMessage("COULD_NOT_FIND_PARENT_DIR"), this._clientVirtualPath), BXDialogGoodbyeWindow.LayoutType.Error, -1);
					return;
				}

				if (!this.IsPostBack)
				{
					this._clientDirectoryName = di.Name;
				}
			...
			newFolderWizard.EnableFormIdReplace = false;			
		}
	...
	{
		var urlPath = BXRoutesHelper.MakeVirtualUrlPathForPagePath(virtualPath);
		BXPublicMenuItemCollection menuItemCol = BXPublicMenu.Menu.GetMenuByUri(menuType, urlPath);
	...
	protected override void OnLoad(EventArgs e)
	{
		...
		try
		{
			...
			DescriptionInfoParagraphs.Add(string.Format("<a href=\"{0}?path={1}\">{2}</a>", VirtualPathUtility.ToAbsolute("~/bitrix/admin/FileManNewFolderMvc.aspx"), HttpUtility.UrlEncode(_clientVirtualParentDirPath), GetMessage("CREATE_FOLDER_IN_CONTROL_PANEL")));
	...
	void wizard_ButtonClick(object sender, BXPageAsDialogButtonClickEventArgs e)
	{
		try
		{
			...
			string defaultPagePath = BXPath.Combine(clientVirtualSavingPath, _defaultPageName + ".cshtml");

			try
			{
				try
				{					
					BXSecureIO.FileWriteAllText(
						defaultPagePath, 
						Bitrix.Mvc.UI.Admin.VirtualSite.Templates.Razor.BuildCshtml(!string.IsNullOrEmpty(_clientTitle) ? _clientTitle : GetMessage("DEFAULT_TITLE")), 
						BXConfigurationUtility.DefaultEncoding
					);

					BXSecureIO.FileWriteAllText(
						BXRoutesHelper.ReplaceExtension(defaultPagePath, ".cshtml", ".bxc"), 
						Bitrix.Mvc.UI.Admin.VirtualSite.Templates.BXC.GetEmptyBXC(), 
						BXConfigurationUtility.DefaultEncoding
					);
				}
				catch
				...
				if (chbxEditAfterSave.Checked)
				{
					SwitchToDialog(
						string.Format(
							"{0}?path={1}&clientType=WindowManager&returnUrl={2}&forcedRedirection=&noundo=",
							VirtualPathUtility.ToAbsolute("~/bitrix/dialogs/VisualPageEditorMvc.aspx"),
							HttpUtility.UrlEncode(defaultPagePath),
							HttpUtility.UrlEncode(BXRoutesHelper.MakeAbsoluteUrlPathForPagePath(VirtualPathUtility.AppendTrailingSlash(clientVirtualSavingPath)))
						),
						null,
			...
			Redirect(BXRoutesHelper.MakeAbsoluteUrlPathForPagePath(defaultPagePath), GetMessage("NEW_FOLDER_IS_SUCCESSFULLY_CREATED_REDIRECTION"), BXDialogGoodbyeWindow.LayoutType.Success);			
		}
		catch (System.Threading.ThreadAbortException /*ex*/)
		{
			//...игнорируем, вызвано Close();
