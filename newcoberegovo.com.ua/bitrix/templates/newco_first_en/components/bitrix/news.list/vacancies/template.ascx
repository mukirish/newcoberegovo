...
{%>
    ...
    <li id="<%= itemContainerId %>" class="inline<%= first_el?" first":"" %>" href="#inline_content_<%=item.ElementId%>"><%
        ...
        <div class="last_a">подробнее</div>
                    ...
                    <div class="block_top"><%
                    	if (item.Properties != null) {%>
                            ...
                            foreach (KeyValuePair<string, Bitrix.BXCustomProperty> kvp in item.Element.Properties){
                                switch (kvp.Key) {
                                    case "EXPERIENCE":%>
                                        <li>Опыт:<img src="/bitrix/templates/newco_first/img/vacan1.png" alt=""/><span><%= kvp.Value.Value%></span></li><%
                                        break;
                                    case "EDUCATION":%>
                                        <li>Образование:<img src="/bitrix/templates/newco_first/img/vacan2.png" alt=""/><span><%= kvp.Value.Value%></span></li><%
                                        break;
                                    case "TIMETABLE":%>
                                        <li>График:<img src="/bitrix/templates/newco_first/img/vacan3.png" alt=""/><span><%= kvp.Value.Value%></span></li><%
                                        break;
                                }
                            }%><div class="clear"></div>
                            </ul><%
