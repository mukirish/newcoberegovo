	...
	<div class="blog-comment-fields">
		...
		<div class="blog-comment-field blog-comment-field-captcha">
			...
			<div class="blog-comment-field-captcha-image">
				<img id="<%= ClientID %>_captcha" alt="Captcha" src="<%= Hidden ? "" : CaptchaHref %>" />
			</div>
		</div>
		<% } %>
...
<script type="text/javascript">
	...
	{		
		...
		if (visible) 
		{
			div.style.display = "";
			<% if (RequireCaptcha) { %>
			var captcha = document.getElementById('<%= ClientID %>_captcha');
			if (captcha != null && !captcha.getAttribute('src')) 
			{
				captcha.setAttribute('src', '<%= JSEncode(CaptchaSrc) %>');
			}
			<% } %>
		}
	}
