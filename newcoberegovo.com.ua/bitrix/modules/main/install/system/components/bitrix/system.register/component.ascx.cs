	...
	public partial class SystemRegisterComponent : BXComponent
	{
			...
			switch (status)
			{
				...
				case MembershipCreateStatus.InvalidPassword:
					return GetMessage("Error.PasswordContraveneSiteRules") +
						(BXUserManager.Provider.MinRequiredPasswordLength > 0 ? string.Format(GetMessage("Error.FormatPasswordMustNotBeShorterThan"), BXUserManager.Provider.MinRequiredPasswordLength) : string.Empty) +
						(BXUserManager.Provider.MinRequiredNonAlphanumericCharacters > 0 ? string.Format(GetMessage("Error.FormatPasswordMustContain"), BXUserManager.Provider.MinRequiredNonAlphanumericCharacters) : string.Empty);
		...
		public override bool ProcessCommand(string commandName, BXParamsBag<object> commandParameters, List<string> commandErrors)
		{
			...
			else if (commandName.Equals("linkToOpenId", StringComparison.InvariantCultureIgnoreCase))
			{
				...
				{
					BXUser user = BXUserManager.GetByName(login, providerName, false);
