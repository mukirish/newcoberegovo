<%@ Page Language="C#" MasterPageFile="~/bitrix/admin/AdminMasterPage.master" AutoEventWireup="true"
	CodeFile="FileManMvc.aspx.cs" Inherits="bitrix_admin_FileManMvc" Title="<%$ Loc:PageTitle %>"
	...
	<asp:UpdatePanel runat="server" ID="UpdatePanel1">
		<ContentTemplate>
			...
			<script type="text/javascript">
				...
				{
					location.href = '<%= BXJSUtility.Encode("FileManEditMvc.aspx?path=" + HttpUtility.UrlEncode(curPath)) %>&new=';
				}
				...
				{
					//<%= selectDirDialogMvc.JavaScriptObject %>.Reset();//CollapseToRoot();
					//<%= selectDirDialogMvc.JavaScriptObject %>.ExpandPath('<%= BXJSUtility.Encode(curPath) %>');
					<%= selectDirDialogMvc.ContainerDialog.GetJSObjectName() %>.ShowPopupDialog(false, true);
					return false;
				}
				function fileMan_SelectDirOK()
				{
					document.getElementById('<%= targetFolder.ClientID %>').value = <%= selectDirDialogMvc.JavaScriptObject %>.GetTargetValue();
					<%= selectDirDialogMvc.ContainerDialog.GetJSObjectName() %>.ClosePopupDialog();
					return false;
				}
			...
			<%
				NewFolderButton.Href = String.Concat("FileManNewFolderMvc.aspx?path=", HttpUtility.UrlEncode(curPath));
				NewFileButton.Href = String.Concat("FileManEditMvc.aspx?path=", HttpUtility.UrlEncode(curPath), "&new=");
			...
			<bx:BXContextMenuToolbar ID="Toolbar1" runat="server" >
				<Items>
					...
					<bx:BXCmImageButton ID="NewFolderButton" LinkButtonClientID="btn_new_folder" CssClass="context-button icon" CommandName="newfoldermvc" Text="<%$ Loc:ActionText.NewFolder %>" Title="<%$ Loc:ActionTitle.NewFolder %>" />
					<bx:BXCmSeparator />
					<bx:BXCmImageButton ID="NewFileButton" LinkButtonClientID="btn_new_file" CssClass="context-button icon" CommandName="newfilemvc" Text="<%$ Loc:ActionText.NewFile %>" Title="<%$ Loc:ActionTitle.NewFile %>" />
					<bx:BXCmSeparator />
			...
			<bx:DirectoryBrowser ID="selectDirDialogMvc" runat="server" ShowFiles="false" ItemsToSelect="Folders" EnableExtras="false" ShowDescription="true"
