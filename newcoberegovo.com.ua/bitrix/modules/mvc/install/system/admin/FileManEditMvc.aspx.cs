...
using Bitrix.IO;
using Bitrix.Mvc;
using Bitrix.Mvc.UI.Admin.VirtualSite.Templates;
...
//using BXSecureIO = Bitrix.Mvc.IO.BXSecureIO;
using Bitrix.DataLayer;

public partial class bitrix_admin_FileManEditMvc : Bitrix.UI.BXAdminPage, IBXFileManPage
{
	...
	private bool isNew;
	private bool isCshtml;
	private bool isCshtmlRaw;
			...
			if (goBackUrl == null)
			{
				try
				{
					...
					if (BXSecureIO.DirectoryExists(curPath))
						return (goBackUrl = "FileManMvc.aspx?path=" + HttpUtility.UrlEncode(curPath));
					if (!Bitrix.IO.BXPath.BreakExistingPath(curPath, ref dir, ref entity))
						return (goBackUrl = "FileManMvc.aspx");
					goBackUrl = "FileManMvc.aspx?path=" + HttpUtility.UrlEncode(dir);
				}
				catch
				{
					return "FileManMvc.aspx";
				}
			}
			return goBackUrl;
		}
	...
	public override void ProcessRequest(HttpContext context)
	{
		if (context.Request["check"] != null)
		{
			...
			try
			{
				...
				string path = Bitrix.IO.BXPath.ToVirtualRelativePath(context.Request["path"]);
	...
	protected void Page_LoadComplete(object sender, EventArgs e)
	{
		...
		finally
		{
			...
			MasterTitle = string.Format(
				...
				GetMessage(this.isCshtml ? "MasterTitle.PageEdit" : "MasterTitle.TextEdit"),
				Encode(curPath)
			);
		}
	}
	...
	protected void SaveClick(object sender, EventArgs e)
	{
		...
		{
			Response.Redirect(BXRoutesHelper.MakeAbsoluteUrlPathForPagePath(BXRoutesHelper.ReplaceExtension(this.savePath, ".cshtml", ".bxc")));
		}
		else
		{
			this.GoBack();
	...
	//For Mvc
	/*protected string MakeBackUrl(string path)
	{
		var back = this.BackUrl;
		if (!back.Contains("path="))
		{
			back = back + (back.Contains("?") ? "&path=" + path : "?path=" + path);
		}
		return back;
	} */
	...
	protected void ApplyClick(object sender, EventArgs e)
	{
		...
		this.Reload(BXTabControl1.SelectedIndex, "path=" + HttpUtility.UrlEncode(curPath));		
	}
	...
	protected void MenuTypeOptions_ItemDataBound(object sender, RepeaterItemEventArgs e)
	{
		...
		if (!String.IsNullOrEmpty(pair.Key) && (pair.Key.IndexOfAny(Path.GetInvalidFileNameChars()) == -1))
			try
			{
				string menuPath = Bitrix.IO.BXPath.Combine(this.curDir, pair.Key + ".menu");
	...
	protected void VisualEditor_IncludeWebEditorScript(object sender, BXWebEditor.IncludeWebEditorScriptArgs e)
	{
		...
		{
			int i = 0;
			string fileTemplate = Bitrix.IO.BXPath.Combine(this.curDir, "Default");
		...
		string v = Bitrix.IO.BXFile.GetFileTimestamp(Bitrix.IO.BXPath.MapPath("~/bitrix/controls/Main/editor/js/toolbar_aspx.js")).ToString();
		...
		v = Bitrix.IO.BXFile.GetFileTimestamp(Bitrix.IO.BXPath.MapPath("~/bitrix/controls/Main/editor/js/FileManEdit_editor.js")).ToString();
	...
	private string MakeEditFileUrl(string filePath)
	{
		...
		if (!this.SaveAs.Text.Equals("default.cshtml", StringComparison.InvariantCultureIgnoreCase))
		{
			page = string.Format("/{0}", this.SaveAs.Text);
		}

		UriBuilder uri = new UriBuilder(Request.Url);
		BXQueryString query = BXUri.GetQueryString(Request.Url.ToString());
		query["path"] = filePath + page;
		query.Remove("new");
	...
	private void SelectContentEditor()
	{
		...
		//if (editor.Equals("visual", StringComparison.InvariantCultureIgnoreCase) && isCshtml)
		//{
	...
	{
		BXSite site = null;
		string path = Bitrix.IO.BXPath.ToVirtualRelativePath(Request["path"]);
		
		site = BXSite.GetSitesForPath(path).FirstOrDefault() ?? BXSite.DefaultSite;
		this.siteId = site.Id;

		BXPath.BreakPath(path, ref this.curDir, ref this.curFileName);
		this.curPath = path;

		/*if (!BXRoutesHelper.IsArea(path))
		{
			
			this.curDir = Bitrix.IO.BXPath.ToVirtualRelativePath(BXRoutes.Instance.GetDirectoryPathByUrl(site.Id, path));
			this.curPath = Bitrix.IO.BXPath.ToVirtualRelativePath(BXRoutes.Instance.GetPagePathByUrl(site.Id, path));

			if (this.curDir.Equals("~"))
			{
				this.curDir = BXPath.GetDirectory(string.Format("~/sites/{0}/{1}", site.Id, path));
			}

			if (path.StartsWith(this.siteId + "/"))
			{
				path = path.Remove(0, (this.siteId + "/").Length);
			}

			if (this.curPath.Equals("~"))
			{
				this.curPath = string.Format("~/sites/{0}/{1}", site.Id, path);
			}
		}
		else
		{
			this.curDir = string.Format("~/sites/{0}", BXPath.GetDirectory(path));
			...
			this.curPath = Bitrix.IO.BXPath.ToVirtualRelativePath(string.Format("~/sites/{0}/{1}", site.Id, path));
		}*/
		...
		if (isNew)
		{
			try
			{
				string filePath = HostingEnvironment.MapPath(this.curPath);
				...
				if (!isFileNameDefined || isDirectoryExist)
				{
					...
					do
					{
						workFile = Path.Combine(workDir, "Default" + (i == 0 ? string.Empty : i.ToString()) + ".cshtml");
						i++;
					}
					while (File.Exists(workFile));

					curPath = Bitrix.IO.BXPath.ToVirtualRelativePath(workFile);
				}
			}
			catch
			{
		...
		if (!Bitrix.IO.BXPath.BreakPath(this.curPath, ref this.curDir, ref this.curFileName))
			throw new PublicException(GetMessageRaw("Message.InvalidDirectory"));

		curSiteId = site == null ? null : site.Id;

		Bitrix.IO.BXFileType fileType = Bitrix.IO.BXFileInfo.GetFileType(curPath);
		if (!curPath.EndsWith(".cshtml"))
		{
			if ((fileType & Bitrix.IO.BXFileType.TextFormat) == Bitrix.IO.BXFileType.Unknown)
				throw new PublicException(GetMessageRaw("Message.UnknownFormat"));
		}
		
		this.isCshtml = true;
		if (this.isCshtml && Request.QueryString["raw"] != null)
		{
			this.isCshtml = false;
			this.isCshtmlRaw = true;
		}
	}
	...
	{		
		...
		vlrSaveAs.ValidationExpression = Bitrix.IO.BXPath.NameValidationRegexString;
	...
	{
		//PageTitle.Text = "Untitled Page";
		if (isNew)
		{
			if (this.isCshtml || this.isCshtmlRaw)
			{
				contentEditor.Content = Bitrix.Mvc.UI.Admin.VirtualSite.Templates.Razor.BuildCshtml("Untitled Page", curPath, null); //BXDefaultFiles.BuildAspx("Untitled Page", curPath, null);
			}
		}
		else
		{
			...
			string bxcPath = BXRoutesHelper.ReplaceExtension(this.curPath, ".cshtml", ".bxc");

			var keywords = new BXParamsBag<string>();

			if (BXSecureIO.FileExists(bxcPath))
			{
				try
				{
					var data = BXComponent2Loader.LoadComponentsData(HostingEnvironment.MapPath(bxcPath));
	...
	private void PrepareEditor()
	{
		...
		newPageOptions.Visible = isNew && this.isCshtml;
		settingsTab.Visible = this.isCshtml;
		//EditPageTitle.Visible = isCshtml;

		if (this.isCshtml)
			PreparePageEditor();

		mainTab.Title = GetMessage(this.isCshtml ? "TabTitle.PageEdit" : "TabTitle.TextEdit");
	}
	...
	private void ValidateSave()
	{
		...
		this.savePath = Bitrix.IO.BXPath.Combine(saveDir, saveFileName);
		...
		string saveAsPath = Bitrix.IO.BXPath.TrimEnd(Bitrix.IO.BXPath.ToVirtualRelativePath(Request.Form[SaveAsPath.UniqueID] ?? "~"));
		this.saveDir = Bitrix.IO.BXPath.GetDirectory(saveAsPath);
		this.savePath = Bitrix.IO.BXPath.Combine(saveDir, saveFileName);
	...
	private void SaveMenu()
	{
		...
		{
			string menuType = MenuType.SelectedValue;
			string menuPath = Bitrix.IO.BXPath.Combine(this.saveDir, menuType + ".menu");
			BXSecureIO.DemandView(menuPath);
			...
			if (menuControls[menuType].New.SelectedValue != "exists") //new
			{
				...
				item.Link = BXSiteRemapUtility.UnmapVirtualPath(Bitrix.IO.BXPath.ToVirtualRelativePath(this.savePath));
				menu.Insert(j, item);
			...
			else
			{
				...
				if (j >= 0)
				{
					...
					item.Links.Add(BXSiteRemapUtility.UnmapVirtualPath(Bitrix.IO.BXPath.ToVirtualRelativePath(savePath)));
					for (int k = 0; k < menu.Count; k++)
						menu[k].Sort = (k + 1) * 10;
					BXPublicMenu.Menu.Save(this.saveDir, menuType, menu);
				}
	...
	private void SaveContent()
	{
		...
		try
		{
			...
			if (this.isCshtml)
			{
				...
				foreach (var item in keywordsControls)
				{
					metatags.Add(item.Key, item.Value.Text);
				}
				
				string bxcPath = BXRoutesHelper.ReplaceExtension(this.savePath, ".cshtml", ".bxc");

				var keywords = this.GetPageSettings();

				BXComponentXmlItems data = 
					!BXSecureIO.FileExists(bxcPath) 
				    ? new BXComponentXmlItems() 
				    : BXComponent2Loader.LoadComponentsData(HostingEnvironment.MapPath(bxcPath));

				data.Attributes = keywords.ToList((k, v) => new BXContainerAttributeXmlItem { Id = k, Value = v });
				BXSecureIO.FileWriteAllText(bxcPath, data.ToXml(), BXConfigurationUtility.DefaultEncoding);
			}
			else
				BXSecureIO.FileWriteAllText(this.savePath, content, BXConfigurationUtility.DefaultEncoding);
		}
	...
	{
		TextBox content;
		bitrix_admin_FileManEditMvc page;

		public TextBoxContentEditor(TextBox content, bitrix_admin_FileManEditMvc page)
		{
		...
		public string Content
		{
			get
			{
				...
				if (!string.IsNullOrEmpty(c) && page.isCshtml)
				{
	...
	private class VisualContentEditor : IContentEditor
	{
		...
		public VisualContentEditor(BXWebEditor content, bitrix_admin_FileManEditMvc page)
		{
			this.content = content;
			this.page = page;
		}
	...
	public string ProvidePath()
	{
		...
		if (!Bitrix.IO.BXPath.BreakPath(path, ref dir, ref file))
			return "~";
		return Bitrix.IO.BXPath.ToVirtualRelativePath(dir).ToLowerInvariant();
	}
