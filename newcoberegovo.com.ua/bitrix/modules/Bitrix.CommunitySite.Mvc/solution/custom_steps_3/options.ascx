...
<script runat="server">
	...
	protected override BXWizardResult OnActionNext(Bitrix.DataTypes.BXCommonBag parameters)
	{
		...
		foreach(var p in options)
			BXOptionManager.SetOptionString("Bitrix.CommunitySite.Mvc", p.Key, p.Value, site.Id);
		...
		var h = WizardContext.State.Get<Bitrix.DataTypes.BXParamsBag<object>>("Bitrix.CommunitySite.Mvc.Settings").GetString("SiteName");
		...
		UI.SetProgressBarValue("Installer.ProgressBar", "Bitrix.CommunitySite.Mvc", 5);
		return Result.Next();
	}
</script>
Настройка решения
