...
public partial class SettingsStep : BXWizardStepStandardHtmlControl
{
	...
	protected const string Key = "Bitrix.CommunitySite.Mvc.Settings";
	...
	protected override void OnWizardInit()
	{
		...
		var header = BXPath.MapPath(site.DirectoryVirtualPath + "area/_area_logo_text.cshtml");
		parameters["Header"] = File.Exists(header) ? File.ReadAllText(header, Encoding.UTF8) : "<h1>Велосипедисты</h1>\r\n<span>клуб любителей велосипедов</span>";
		parameters["SiteName"] = File.Exists(header) && site != null ? site.Name : "Клуб любителей велосипедов";

		var footer = BXPath.MapPath(site.DirectoryVirtualPath + "area/_area_copyright.cshtml");
		parameters["Copyright"] = File.Exists(footer) ? File.ReadAllText(footer, Encoding.UTF8) : "© Название сайта, 2010";
	...
	private BXWizardResult PrepareView(List<string> errors)
	{
		...
		if (template == "taby")
		{
			...
			var currentLogo = BXPath.MapPath(site.DirectoryVirtualPath + "area/_area_logo_image.cshtml");
			var defaultLogo = VirtualPathUtility.ToAbsolute(BXPath.Combine(solutionPath, "templates/taby/images/logo.jpg"));
			...
			else if (File.Exists(currentLogo))
			{
				...
				logoSrcPath = logoSrcPath.Replace("@(BXSite.CurrentTemplate)", solution + "." + template + "." + site.Id);
				Uri url;
	...
	protected override BXWizardResult OnActionNext(Bitrix.DataTypes.BXCommonBag parameters)
	{
		UI.LoadValues(parameters);
		UI.Overwrite(Key);
		UI.SetProgressBarMaxValue("Installer.ProgressBar", "Bitrix.CommunitySite.Mvc", 7);
